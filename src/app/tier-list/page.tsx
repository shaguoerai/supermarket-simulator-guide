  ...
  return (
    <div className="flex flex-col min-h-screen bg-zinc-950 text-zinc-100">
      ...
      <main className="flex-1 max-w-4xl mx-auto px-6 py-12 w-full">
        ...
        <p className="text-zinc-400 mb-8 text-lg">All products ranked with category, margin, and license requirements.</p>

        <div className="flex flex-wrap gap-3 mb-8">
          <div className="inline-flex items-center gap-2 px-3 py-1.5 rounded-full bg-zinc-900 border border-zinc-800 text-sm text-zinc-400">
          ...
          {["S", "A", "B", "C"].map((tier) => (
            <section key={tier}>
              <div className="flex items-center gap-3 mb-3">
                <Trophy className={`w-5 h-5 ${tier === "S" ? "text-yellow-400" : tier === "A" ? "text-emerald-400" : tier === "B" ? "text-blue-400" : "text-zinc-400"}`} />
                <h2 className={`text-xl font-bold ${tier === "S" ? "text-yellow-400" : tier === "A" ? "text-emerald-400" : tier === "B" ? "text-blue-400" : "text-zinc-400"}`}>Tier {tier}</h2>
              </div>
              <div className="grid gap-2">
                {products.filter((p) => p.tier === tier).map((p) => (
                  <div key={p.name} className={`p-3 rounded-lg border flex items-center justify-between ${tierColors[tier]}`}>
                    <div>
